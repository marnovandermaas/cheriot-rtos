...
class SonataPinmux : private utils::NoCopyNoMove {
  ...
  /**
   * A helper that returns a pointer to Pin Multiplexer's registers,
   * where each sequential byte potentially corresponds to some mapped
   * pinmux selector, used to select the pin configuration.
   */
  [[nodiscard, gnu::always_inline]] volatile uint8_t *registers() const
  {
    return MMIO_CAPABILITY(uint8_t, pinmux);
  }
